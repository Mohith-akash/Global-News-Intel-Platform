...
def get_db_connection():
    """Native DuckDB connection for UI visuals"""
...
# [IMPROVED HEADLINE CLEANER]
def format_headline(url, actor):
    if not url: return f"Intel Report: {actor}"
    try:
        path = urlparse(url).path
        slug = path.rstrip('/').split('/')[-1]
        
        # Fallback to previous path segment if slug is too short/numeric
        if len(slug) < 5 or slug.isdigit() or 'index' in slug.lower():
            slug = path.rstrip('/').split('/')[-2]

        text = slug.replace('-', ' ').replace('_', ' ').replace('+', ' ')
        
        # 1. Remove Extensions & Junk
        text = re.sub(r'\.html?$', '', text)
        text = re.sub(r'\b\d{8,}\b', '', text) # Remove long number strings
        text = re.sub(r'[A-Za-z0-9]{15,}', '', text) # Remove mixed hashes
        
        # 2. Remove Dates (2025 11 19)
        text = re.sub(r'\b20\d{2}[\s/-]?\d{1,2}[\s/-]?\d{1,2}\b', '', text)

        # 3. Remove "Clickbait" Prefixes
        text = re.sub(r'^(article|story|news|report|default|page)\s*', '', text, flags=re.IGNORECASE)
        ...
        # 5. Last Resort Check
        if len(headline) < 5:
            return f"Situation Update: {actor.title()}"
...
def render_chat_section(conn_ui, engine_ai):
    ...
    if user_input:
        if not st.session_state.get('llm_locked', False):
                ...
                try:
                    ...
                    if qe:
                        ...
                        if hasattr(response_obj, 'metadata') and 'sql_query' in response_obj.metadata:
                            ...
                            # Simple safety check
                            if "drop" not in sql.lower() and "delete" not in sql.lower():
    ...
    # Show History (Newest Top, Limit 5)
